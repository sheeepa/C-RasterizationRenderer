...
bool BlinnShader::fragment(shader_struct_v2f* v2f, Color& color) {
	...
	Color albedo = tex_diffuse(v2f->uv) * shader_data->material->color;
	...
	Color spcular = shader_data->light_color * shader_data->material->specular * std::pow(saturate(worldNormalDir * halfDir), shader_data->material->gloss);
...
bool NormalMapShader::fragment(shader_struct_v2f* v2f, Color& color) {
	...
	bump.x = bump.x * shader_data->material->bump_scale;
	bump.y = bump.y * shader_data->material->bump_scale;
